...
class Reserva extends ConexaoDB{
    ...
    function __construct($id, $usuario, $dataInicial, $dataFinal, $imovel, $intermediador, $porcentagemIntermediador, $precoDiaria, $taxaLimpeza, $desconto) {
        $this->id = $id;
    ...
    public function cadastrarReserva(Reserva $reserva) {
        ...
        $sql = "insert into reserva(id_usuario,id_imovel,id_intermediador,data_inicial,data_final,porcentagem_intermediador,preco_diaria,taxa_limpeza,desconto) values (:id_usuario,:id_imovel,:id_intermediador,:data_inicial,:data_final,:porcentagem_intermediador,:preco_diaria,:taxa_limpeza,:desconto)";
    ...
    public static function listarReservas($idUsuario,$idImovel){
        $pdo = new PDO(ConexaoDB::getConexaoStatic());
        $sql = "SELECT imovel.identificacao,reserva.id_intermediador,reserva.id, reserva.data_inicial, reserva.data_final,reserva.porcentagem_intermediador,reserva.preco_diaria,reserva.taxa_limpeza,reserva.desconto
                FROM reserva 
        ...
        $reservas = array();
        foreach ($select as $r) {
            $reserva = new Reserva($r['id'], $idUsuario, $r['data_inicial'], $r['data_final'], $idImovel, $r['id_intermediador'], $r['porcentagem_intermediador'], $r['preco_diaria'], $r['taxa_limpeza'], $r['desconto']);
    ...
    public static function listarReserva($idUsuario,$id){
        ...
        foreach ($select as $r) {
            $reserva = new Reserva($r['id'], $idUsuario, $r['data_inicial'], $r['data_final'], $r['id_imovel'], $r['id_intermediador'], $r['porcentagem_intermediador'], $r['preco_diaria'], $r['taxa_limpeza'], $r['desconto']);
            return $reserva;
        }
    }
    ...
    public function editarReserva(Reserva $reserva){
        $pdo = new PDO($this->getConexao());
        $sql = "update reserva set id_imovel= :id_imovel, id_intermediador= :id_intermediador, data_inicial= :data_inicial, data_final= :data_final, porcentagem_intermediador= :porcentagem_intermediador, preco_diaria= :preco_diaria, taxa_limpeza= :taxa_limpeza, desconto= :desconto where id= :id and id_usuario = :id_usuario;";
        $insert = $pdo->prepare($sql);
